...
% p_games_wo_g14_set = p_games_wo_g14(g12 = ww, g23 = ww, ... ,34 = ww)
...
%% Check that p(p1,p2,p3,p4|Games) != p(p1|Games)x...x p(p4|Games)
p_all = p_p1p2p3p4_given_games;
p_mul = multpots([p1_given_games p2_given_games p3_given_games p4_given_games]);

if (sum(sum(sum(sum(p_all.table==p_mul.table))))<10^4)
    disp('Skills dependent given outcome of the Games');
else
     disp('Skills apriori independent given outcome of the Games');
end;
clear all;